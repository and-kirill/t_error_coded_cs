function chs = rs_single_run(sigma_noise, sys_params, Ka, V, kx)
rs_params = generate_rs_params(sys_params, V, kx);
[~, cwd_bin, cwd_rs, prefix] = rs_encode(sys_params, rs_params, Ka);
...
for K0 = floor(Ka / 2):K0_max
    [success_K, n_CRC] = evaluate_rs_performance(rs_params, cwd_hat_qary(:, 1:K0), cwd_rs.x, prefix);
